...
export class SurveyService {
  private API_BASE = 'https://localhost:7245/api/v1/survey'; // <-- Use versioned API path

  constructor(private http: HttpClient) {}

  createSurvey(survey: SurveyCreateDto): Observable<any> {
    return this.http.post(this.API_BASE, survey);
  }

  getAllSurveys(): Observable<SurveyDto[]> {
    // Use default pagination params to ensure backend returns data
    return this.http.get<SurveyDto[]>(this.API_BASE + '?pageNumber=1&pageSize=100');
  }

  getSurveysPaginated(pageNumber = 1, pageSize = 5): Observable<PaginatedSurveys> {
    // Always request sorted by newest first
    return this.http.get<PaginatedSurveys>(`${this.API_BASE}?pageNumber=${pageNumber}&pageSize=${pageSize}&sortBy=startDate&sortOrder=desc`);
  }

  getSurveyById(id: number) {
    return this.http.get<SurveyDto>(`${this.API_BASE}/${id}`);
  }

  updateSurvey(id: number, survey: any) {
    return this.http.put(`${this.API_BASE}/${id}`, survey);
  }

  // --- Survey Results Analytics ---
  getSurveyResults(surveyId: number): Observable<SurveyResultsDto> {
    return this.http.get<SurveyResultsDto>(`${this.API_BASE}/${surveyId}/results`);
  }
